...
namespace sge {
	...
	struct Vec4_Basic : public DATA {
		...
		SGE_INLINE Vec4 operator+ (const Vec4& v) const { return Vec4(x + v.x, y + v.y, z + v.z, w + v.w); }
		SGE_INLINE Vec4 operator- (const Vec4& v) const { return Vec4(x - v.x, y - v.y, z - v.z, w - v.w); }
		SGE_INLINE Vec4 operator* (const Vec4& v) const { return Vec4(x * v.x, y * v.y, z * v.z, w * v.w); }
		SGE_INLINE Vec4 operator/ (const Vec4& v) const { return Vec4(x / v.x, y / v.y, z / v.z, w / v.w); }
		...
		SGE_INLINE void operator+=(const Vec4& v) { x += v.x; y += v.y; z += v.z; w += v.w; }
		SGE_INLINE void operator-=(const Vec4& v) { x -= v.x; y -= v.y; z -= v.z; w -= v.w; }
		SGE_INLINE void operator*=(const Vec4& v) { x *= v.x; y *= v.y; z *= v.z; w *= v.w; }
		SGE_INLINE void operator/=(const Vec4& v) { x /= v.x; y /= v.y; z /= v.z; w /= v.w; }
		...
		SGE_INLINE bool operator==(const Vec4& v) { return x == v.x && y == v.y && z == v.z && w == v.w; }
		SGE_INLINE bool operator!=(const Vec4& v) { return x != v.x || y != v.y || z != v.z || w == v.w; }
