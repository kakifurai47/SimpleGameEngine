...
namespace sge {
	...
	public:
		...
		SGE_INLINE Vec3_Basic()			   	 = default;
		SGE_INLINE Vec3_Basic(const Vec3& v) = default;
		SGE_INLINE Vec3_Basic(const Tuple3<T> & v)					 	{ set(v); }
		SGE_INLINE Vec3_Basic(const Vec2& v, const T& z_)				{ set(v.x, v.y, z_); }
		SGE_INLINE Vec3_Basic(const T& x_, const T& y_, const T& z_) 	{ set(x_,  y_,  z_); }
	...
	//----
		SGE_INLINE Vec3 operator-() { return Vec3(-x, -y, -z); }
